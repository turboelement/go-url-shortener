...
func (r *PostgresRepository) Stats(ctx context.Context) (StatsResult, error) {
	...
	`).Scan(&result.URLs, &result.Users)
	if err != nil {
		return StatsResult{}, fmt.Errorf("error getting stats: %w", err)
	}
	return result, nil
}
